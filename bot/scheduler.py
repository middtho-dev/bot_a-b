...
from bot.texts import BTN_FILL_EOD_PRIVATE, SCHEDULER_TEXTS
...
async def _send_checkin_prompt(bot: Bot, settings: Settings, db: Database) -> None:
    ...
    if not general_chat_id:
        logger.warning(SCHEDULER_TEXTS["general_chat_not_set"])
        return

    kb = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text=SCHEDULER_TEXTS["checkin_button"], callback_data=f"checkin:{datetime.now(settings.timezone).date().isoformat()}")]]
    )
    await bot.send_message(
        general_chat_id,
        SCHEDULER_TEXTS["checkin_prompt"],
        reply_markup=kb,
    )
...
async def _send_checkin_missing(bot: Bot, settings: Settings, db: Database) -> None:
    ...
    await bot.send_message(admin_chat_id, build_missing_report(SCHEDULER_TEXTS["checkin_missing_title"], today, missing))
...
async def _send_eod_prompt(bot: Bot, settings: Settings, db: Database) -> None:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=BTN_FILL_EOD_PRIVATE, url=url)]])
    await bot.send_message(general_chat_id, SCHEDULER_TEXTS["eod_prompt"], reply_markup=kb)
...
async def _send_eod_missing(bot: Bot, settings: Settings, db: Database) -> None:
    ...
    await bot.send_message(admin_chat_id, build_missing_report(SCHEDULER_TEXTS["eod_missing_title"], today, missing))
...
async def _check_inactivity(bot: Bot, settings: Settings, db: Database) -> None:
    ...
    for uid, emp in settings.employees.items():
        ...
        if (now - last_dt).total_seconds() >= minutes * 60:
            await bot.send_message(
                admin_chat_id,
                SCHEDULER_TEXTS["inactivity_alert"].format(minutes=minutes, user=emp.username or uid, last_time=last_dt.strftime("%H:%M")),
            )
