...
def build_weekly_report(end_day: date, rows: list[DailyActivity], employees: dict[int, Employee]) -> str:
    ...
    lines = [f"🗓 Weekly activity report — {start_day.isoformat()} .. {end_day.isoformat()}", ""]
    for user_id, employee in employees.items():
        counters = by_user_day.get(user_id, {})
        total = sum(counters.values())
        avg = round(total / 7, 2)
        silent_days = sum(1 for d in days if counters.get(d, 0) == 0)
        lines.append(f"{employee.role.capitalize()} {employee.full_name}: total={total}, avg/day={avg}, no_activity_days={silent_days}")
    return "\n".join(lines)
...
def build_kpi_block(
    ...
    lines = [f"📈 KPI snapshot {start_day}..{end_day}", "Sales:"]
    for uid, emp in employees.items():
        if emp.role != "sales":
            continue
        item = sales_by_user.get(uid, {"count": 0, "sum": 0.0})
        lines.append(f"- {emp.full_name}: deals={int(item['count'])}, amount={item['sum']:.2f} AED")
    ...
    for uid, emp in employees.items():
        ...
        lines.append(f"- {emp.full_name}: shipments={item['count']}, delayed={item['delayed']}")
