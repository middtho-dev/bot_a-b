...
## Как считаются сообщения и куда уходят отчёты

### Куда отправляются отчёты
- Автоматические дневной/недельный отчёты отправляются в `ADMIN_CHAT_ID`.
- После отправки они остаются обычными сообщениями в истории этого чата (бот их не удаляет).
- Вручную отчёты можно запросить командами `/report today` и `/week`.

### Как считается активность
- Бот учитывает сообщения только сотрудников, которые есть в системе.
- Учитываются **обычные сообщения в рабочих чатах**, без обязательных обращений к боту (`@bot`, reply, команды не нужны).
- Если `WORK_CHAT_IDS` не задан, бот использует fallback: `GENERAL_CHAT_ID`, `SALES_CHAT_ID`, `LOGISTICS_CHAT_ID`.
- Команды вида `/...` в активность не считаются.
- Для отчётов сохраняются только метаданные (количество, первая/последняя активность), без текста сообщений.

> Если в отчёте везде 0, обычно причина — сотрудник не добавлен, chat_id не настроен, бот не видит сообщения из-за режима privacy в BotFather, или сообщения шли не в рабочий чат.
...
Чтобы было удобнее поддерживать проект, все пользовательские тексты, подписи кнопок и шаблоны вынесены в отдельный модуль:
