...
- `/admin` — открывает главное админ-меню (без длинной справки в сообщении).
...
- ⚙️ Меню переменных: каждая переменная открывается в отдельной карточке с описанием, текущим значением и кнопкой «Установить»
  - статус в списке: ⚪ не задано / ⭐ установлено
- 👥 Меню сотрудников: список сотрудников + мастер добавления (ID → имя → username)
- 📘 Отдельная кнопка с полной справкой по командам
