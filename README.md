...
- `/export csv`
- `/admin_test` (временное меню тестирования: имитация check-in/eod/report/inactivity)

> Временные настройки через `/set_*` пишутся в БД и учитываются после перезапуска бота.

> Для чистоты чата бот старается удалять служебные команды и короткие технические ответы.
