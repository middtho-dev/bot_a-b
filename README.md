...
#### 🌆 Вечерний отчёт (EOD)
- В `EOD_TIME` бот отправляет напоминание.
- Пользователь запускает `/eod` и проходит форму в 4 шага:
  1. Сделано сегодня
  2. В работе
  3. Проблемы
  4. Нужна помощь
...
- `/admin_test` — временная тест-панель (см. ниже).
...
Открывает inline-меню для мгновенного вызова сценариев без ожидания cron-времени:
- 🚀 Check-in prompt
- 📋 Check-in missing
- 🌆 EOD prompt
- 📭 EOD missing
...
5. 🧪 На каждом шаге прогонять `/admin_test`
